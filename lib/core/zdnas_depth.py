...
#######################################
def train_epoch_zdnas_depth(epoch, model, zc_func, theta_optimizer, cfg, device, task_flops, 
    ...
    if local_rank in [-1, 0]:
        logger.info(('%10s' * 9) % ('Epoch', 'gpu_num', 'Param', 'FLOPS', 'Layers', 'f_loss', 'zc_loss', 'total', 'temp'))
    ...
    for iter_idx in pbar:
        ...
        output_layers = nn_model.calculate_layers_new(architecture_info)
        ...
        loss = zc_loss + flops_loss + layers_loss
        ...
        avg_layers.update(output_layers.item(), 1)
        ...
        if local_rank in [-1, 0]:
            ...
            s = ('%10s' * 2 + '%10.4g' * 7) % (
                '%3d/%3d' % (epoch,cfg.EPOCHS), mem, avg_params.avg, avg_flops.avg, avg_layers.avg, avg_floss.avg, \
    ...
    print(f'Continuous Current FLOPS: {output_flops:.2f}G   Params: {output_params:.2f}M   Layers: {output_layers}   ZC: {zc_score}')
    ...
    output_layers = nn_model.calculate_layers_new(architecture_info)
    zc_score = model.calculate_zc(architecture_info, zc_map)
    print(f'Discrete Current FLOPS: {output_flops:.2f}G   Params: {output_params:.2f}M   Layers: {output_layers}   ZC: {zc_score}')
