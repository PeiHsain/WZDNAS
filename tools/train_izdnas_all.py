...
from lib.utils.latency_lookup_table import LatencyEstimator
...
from lib.core.zdnas_all import train_epoch_zdnas_all
...
def config_backup(config_bakup_dir, code_backup_dir, args):
    ...
    shutil.copy(args.lookup,os.path.join(config_bakup_dir, os.path.basename(args.lookup)))
    with open(os.path.join(config_bakup_dir, 'commandline.txt'), 'w') as f:
        f.writelines(' '.join(sys.argv))
        
    shutil.copy('lib/core/zdnas_all.py',                  os.path.join(code_backup_dir, 'zdnas_all.py'))
...
def parse_config_args(exp_name):
    ...
    parser.add_argument('--lookup', type=str, default='config/lookup/p5_rb5_gpu.yaml', help='lookup_table.yaml path')
    ###################################################################################
...
def main():
    ...
    latency_table = LatencyEstimator(args.lookup)
    ...
    try:
        ...
        # Count pretraining time
        t1 = time.time()
        logger.info(f"[info] Time of pretrain supernet weights = {t1-t}s")
        ...
        for epoch in range(cfg.STAGE2.EPOCHS):
            best_arch = train_epoch_zdnas_all(epoch, model, wot_function, theta_optimizer, cfg, 
                device=device, task_flops=TASK_FLOPS, est=model_est, table=latency_table, logger=logger, logdir=output_dir)
            # train_epoch_zdnas(epoch, model, wot_function, theta_optimizer, cfg, 
